...
import witheppy.ephtml as ephtml
...
""",
    ...
    for html, theline, expected in data:
        ...
        result = ephtml.sniphtml(fhandle)
...
def test_gets2sconversions():
    ...
    for html, expected in data:
        ...
        result = ephtml.gets2sconversions(htables)
        assert result == expected
...
def test_getthetable():
    ...
    for html, header, expected in data:
        htables = readhtml.titletable(StringIO(html))
        result = ephtml.getthetable(htables, header)
        assert result == expected
...
""",
    ...
    for htmltxt, expected in data:
        ...
        result = ephtml.getnexttable(fhandle)
        assert result == expected
...
def test_htmlonetable(tmp_path, htmltxt, header, expected):
    """py.test for htmlonetable"""
    fhandle = StringIO(htmltxt)
    result = ephtml.htmlonetable(fhandle, header)
    assert result == expected
    # -
...
def test_get_upto_nexttable(tmp_path, htmltxt, expected):
    ...
    fhandle = StringIO(htmltxt)
    result = ephtml.get_upto_nexttable(fhandle)
    assert result == expected
    tmp_file = tmp_path / "eplus.htm"
    tmp_file.write_text(htmltxt)
    fhandle = open(tmp_file, "rb")
...
def test_ahtmltable(tmp_path, htmltxt, index, expected):
    """py.test for ahtmltable"""
    fhandle = StringIO(htmltxt)
    result = ephtml.ahtmltable(fhandle, index)
    assert result == expected
    tmp_file = tmp_path / "eplus.htm"
    tmp_file.write_text(htmltxt)
    fhandle = open(tmp_file, "rb")
